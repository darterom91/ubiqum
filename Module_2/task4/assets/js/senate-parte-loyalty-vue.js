...
function totalPCTVotesAsc() {
  ...
  for (var i in objGlobal.results[0].members) {
    if (objGlobal.results[0].members[i].votes_with_party_pct <= 10 && objGlobal.results[0].members[i].votes_with_party_pct != 0) {
      if (cont < numAux) {
...
function totalPCTVotesDesc() {
  ...
  for (var i in app2.members) {
    if (app2.members[i].votes_with_party_pct > 10) {
      if (cont < numAux) {
  ...
  for (var i = 0; i < app2.descs.length; i++) {
    for (var j = 0; j < app2.descs.length - 1; j++) {
      if (app2.descs[i].votes_with_party_pct > app2.descs[j].votes_with_party_pct) {
        aux2 = app2.descs[i].missed_votes_pct;
        app2.descs[i].votes_with_party_pct = app2.descs[j].votes_with_party_pct;
        app2.descs[j].votes_with_party_pct = aux2;
      }
    }
  }
}
